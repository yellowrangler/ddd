...
//---------------------------------------------------------------
$sql = "SELECT id AS memberid,passwd 
FROM membertbl 
...
if ($rc == 1)
{
	...
	if ($count == 1)
	{
		...
		// now build the return full member
		$sql = "SELECT id as memberid, membername, membernameprofileind, screenname, gender, genderprofileind, avatar, role, email, emailprofileind, street, city, state, zip, addressprofileind, phonenumber, phonenumberprofileind, favoriteteamid, noemail, biography, biographyprofileind, status, token, enterdate, createdate 
		FROM membertbl 
		WHERE status = 'active' AND id = '$tblmemberid'";
		// print $sql;
		...
		if (!$sql_result)
		{
			$log = new ErrorLog("logs/");
			$sqlerr = mysql_error();
			$log->writeLog("SQL error: $sqlerr - Error doing final select to db Unable to login for ddd membername $loginmembername.");
			$log->writeLog("SQL: $sql");
		...
		$member = mysql_fetch_assoc($sql_result);
	}
	else
	{
		$rc = -1;
